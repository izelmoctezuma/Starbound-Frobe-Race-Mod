...
function init()
  ...
  quest.setParameter("uniformBox", {type = "item", item = "cardboard2"})
...
function setStage(newStage)
  if newStage ~= self.missionStage then
    if newStage == 1 then
      ...
      setPester("frobestartExitBedPester", 20)
      quest.setObjectiveList({{config.getParameter("descriptions.getDressed"), false}})
    elseif newStage == 2 then
      player.radioMessage("frobestartRoomTutorial2", 1)
	  quest.setIndicators({"uniformBox"})
	  player.radioMessage("frobestartRoomTutorial", 1)
      setPester("frobestartRoomPester", 40)
...
function updateStage(dt)
  ...
  elseif self.missionStage == 2 then
      if hasEquippedUniform() then
        quest.setObjectiveList({{config.getParameter("descriptions.pickUpSign"), false}})
		setStage(3)
      end
...
function hasEquippedUniform()
  ...
  return ((chestItem and chestItem.name == "jumpsuitchest") or
          (chestCosmeticItem and chestCosmeticItem.name == "jumpsuitchest")) and
         ((legsItem and legsItem.name == "jumpsuitlegs") or
          (legsCosmeticItem and legsCosmeticItem.name == "jumpsuitlegs"))
end
